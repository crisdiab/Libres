aplicacion.controller('ClienteModalCtrl', [
  '$scope',
  ...
  function ($scope, $http, $uibModalInstance) {
    ...
    $scope.crearClienteCerrar = function () {
      console.log('llamo a crear cliente cerrar')
            ...
            $http.get(masterUrl+'/ClienteApi')
              .then(
                function success(data) {
                  console.log(data.data);
                  $scope.clientes = data.data;
                  console.log($scope.clientes[$scope.clientes.length - 1].id);
                  $uibModalInstance.close($scope.clientes[$scope.clientes.length - 1]);

                },
                function error(err) {
                  toastr.options.positionClass = 'toast-bottom-center'
                  toastr.warning('El Cliente que ingreso ya existe')
